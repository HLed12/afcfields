...
```
From this subset, it seems that agreement for a field was more difficult throughout sites than agreement for not a field. An explanation could be outliers. Some sites were completely field-free, resulting in the field agreement being 0 and dragging the overall mean down to a lower score. And the reverse for no field agreements at 1 and bringing overall mean up.


#### Summary Analysis Removed Extreme Outliers
```{r, fig.width=6, fig.height=4, fig.align = "center"}
outlier <- c(0, 1)
...
```
Here is the results if we remove sites that are completely field or no field.