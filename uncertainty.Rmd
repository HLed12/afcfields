...
multi_site_list <- lapply(1:nrow(sites_mapped_gt2), function(x) {
  multi_site_test <- sites_mapped_gt2 %>% slice(x)
...
# Process each association
  ...
  multi_site_list[[x]] <- multi_site_assnsr_test
})
...
Next Tasks:
# create a for loop that will do this for every single field and then run analyses on that